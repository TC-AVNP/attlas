#!/usr/bin/env bash
# Build a provisioned golden image for a Pi worker node.
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
OUTPUT="${1:-golden-image-worker.img}"
BASE_IMAGE="/var/lib/homelab-bootstrap/base-worker-arm64.img"
BASE_IMAGE_ZST="/var/lib/homelab-bootstrap/base-worker-arm64.img.zst"
...
echo "PROGRESS:0:Starting worker image build"
...
echo "instance-id: pi-worker-$(date +%Y%m%d%H%M%S)" | sudo tee "${MOUNT}/meta-data" > /dev/null
sudo umount "$MOUNT"
